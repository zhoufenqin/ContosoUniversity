...
Identify all independently deployable services/modules in the project:
...
Additionally identify management and observability endpoints:
...
Identify inter-service and intra-service communication:
...
### Step 5: Build Service Technology Matrix

For each service, identify which cross-cutting capabilities it uses:
...
Create a **Mermaid `sequenceDiagram`** showing the primary request flow across services/layers:
...
[Description of sync vs async patterns, gateway aggregation/composition logic, circuit breaker/retry policies with timeout values, service discovery, startup dependency chain]
...
- Communication patterns section describes sync/async patterns and resilience policies
