...
After assessment completes successfully, you can optionally generate an architecture diagram to visualize the application structure:

**Purpose**: Create a visual representation of:
- Application architecture and components
- Project dependencies
- Technology stack
- Code structure
- Migration considerations (based on assessment findings)

**Output Location**: `.github/appmod/application-diagram.md`

**How to Generate**:
1. Assess the application architecture by analyzing:
   - `*.csproj` or `pom.xml`/`build.gradle` files for project structure
   - `appsettings.json` or `application.properties` for configurations
   - Key code files (Program.cs, Startup.cs, Controllers, etc.)
   - Package/NuGet references for technology stack
   - Project references for dependencies

2. Optionally read `.github/appmod/report.json` for detected issues (if needed for migration notes)

3. Generate a concise markdown file with:
   - **2-3 mermaid diagrams maximum** (architecture overview, project dependencies)
   - **Tables for details** (code structure, technology stack, key classes)
   - **Migration notes** (based on report.json if available)

**Triggering Diagram Generation**:
- User explicitly requests: "Generate architecture diagram"
- Or as a follow-up after assessment: "Can you also create a diagram?"

**Note**: Diagram generation is optional and separate from core assessment. Users can choose to generate it after reviewing the assessment report.
...
Optional diagram is complete when:
• ✅ `.github/appmod/application-diagram.md` is created
• ✅ Contains 2-3 mermaid diagrams showing architecture and dependencies
• ✅ Includes tables describing code structure and technology stack
