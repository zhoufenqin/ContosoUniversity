...
Scan for business process entry points:
...
### Step 2: Trace Business Processes

For each significant entry point, trace the business flow:

- Entry point → service layer → domain logic → persistence
- Identify domain entities and aggregates (DDD patterns if present)
- Map the sequence of operations: validation → business rule check → state mutation → side effects
- Note branching logic (if/else, switch, strategy pattern) that represents business decisions
- Identify orchestration vs choreography patterns in multi-service workflows

**Cross-service data composition flows** — trace end-to-end:
...
- Note which service provides which data and how they are joined/merged
- Document circuit breaker fallback behavior that affects business outcomes (e.g., "when visits-service is unavailable, owner details are returned without visit history" — this is a business-relevant degradation, not just a technical detail)

### Step 3: Map Service Boundaries to Domain Contexts

For microservice or multi-module architectures, map each service/module to its business domain:
...
Extract and document business rules embedded in the code:
...
Document cross-cutting patterns that affect business workflows:
...
### Step 6: Generate Mermaid Sequence Diagram

Create a **Mermaid `sequenceDiagram`** showing the primary business workflow end-to-end:
