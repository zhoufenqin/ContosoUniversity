...
### Step 1: Identify Data Access Layer

Scan source code for data access patterns and ORM frameworks:
...
- Repository/DAO interfaces and implementations — list each repository with its entity type
- Custom query methods beyond basic CRUD (JPQL, LINQ, raw SQL, named queries) — especially bulk/batch query methods (e.g., `findByPetIdIn(Collection<Integer> petIds)`) that enable cross-service data aggregation
...
Extract database configuration from project files, **per profile/environment**:
...
### Step 3: Map Data Ownership Boundaries

Determine data ownership across modules/services as a **structured table**:

- Which module/service owns which tables/entities (bounded context analysis)
- ORM framework used per service
- Caching layer per service (if any)
- Shared vs isolated data stores (shared database, database-per-service, logical separation within shared DB)
- Cross-service data access patterns: document how one service queries another service's data (direct DB access vs REST API calls vs batch/bulk query methods that enable gateway-level aggregation)
- Read/write patterns per module (CQRS if applicable)

### Step 4: Analyze Caching Strategies

Identify caching layers and configuration:
...
### Step 5: Document Repository Interface Inventory

For each service/module, list the key repository interfaces:
...
### Step 6: Generate Mermaid ER Diagram

Create a **Mermaid `erDiagram`** showing entity relationships with cardinality:
